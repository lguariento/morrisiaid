...
declare function app:mainTable($node as node(), $model as map(*), $advancedSearch, $pe1id, $both, $pe2id, $plid, $dateFrom, $dateTo) {
    ...
    if ($advancedSearch eq 'yes') then
        ...
        array {
            ...
            every $condition in (
            if ($pe1id ne '') then
                tei:ab[@type = "sent"]/tei:persName/@ref = $pe1id or
                (if ($both eq 'true') then
                    tei:ab[@type = "received"]/tei:persName/@ref = $pe1id
                else
                    false())
            else
                true(),
            if ($pe2id ne '') then
                tei:ab[@type = "received"]/tei:persName/@ref = $pe2id
            else
                true(),
            if ($plid ne '') then
                tei:ab[@type = 'sent']/tei:placeName/@ref = $plid
            else
                true(),
            
            (: If the date does not have a trailing '-' it means that it's complete :)
            ...
            check if the checkbox 'incomplete' is checked or not. If it's checked, the condition is true and
