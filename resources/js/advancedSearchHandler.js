    ...
    oReq.onreadystatechange = function () {
        if (oReq.readyState === XMLHttpRequest.DONE && oReq.status === 200) {
            ...
            // if it is, just clean the table
            {
                ...
                table.draw()
            } else {for (let [key, value] of Object.entries(result.results)) {
                    if (Array.isArray(value[5])) {
                            value[5] = value[5].join('');
                        }
                        if (Array.isArray(value[4])) {
                            value[4] = value[4].join('');
                        }
                        if (Array.isArray(value[3])) {
                            value[3] = value[3].join('');
                        }
...
function nsResolver(prefix) {
  ...
  return ns[prefix] || null;
    ...
    //} else {
          sessionStorage.setItem('advancedSearch', 'yes');
          // Also clean the URL from any previous parameter.
          window.history.pushState({},
          document.title, "" + "index.html");
          //}
        ...
        // data.append("both", document.getElementById("both").checked?"true":"false");
